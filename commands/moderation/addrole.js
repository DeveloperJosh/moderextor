...
module.exports={
    ...
    description: 'Tired of adding roles manually? Don\'t worry, This bot can help you addrole to a user, swiftly.',
    aliases: ['arole'],
    usage: '.addrole <mention someone> <mention a role>',
    run: async(client, message, args)=>{
        try{
            ...
            if(!role || !member) {
                ...
                \`\`\`fix
.addrole <mention a user> <mention a role or provide the role\'s name>
OR
.addrole <provide the user\'s ID> <provide a role's ID>\`\`\``)
                return message.channel.send(error);
            }
            else if(role && member){
                if(member.roles.cache.has(role.id)) {
                    let me = new Discord.MessageEmbed()
                    .setTitle(`🔴 ${member.user.tag}, already has the role!`)
                    .setColor('#2f3136')
                ...
                .setColor('#2f3136')
                .setDescription(`🟢 **${member.user.tag}** has been given ${role} role`)
